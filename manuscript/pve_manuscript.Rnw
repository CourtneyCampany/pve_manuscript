...
colnames(pve_data) <- c("Volume (L)", 
                        "Seedling~mass~(g)", 
                        "SLA~(m\\textsuperscript{2}~kg\\textsuperscript{-1})",
                        "Leaf~Starch~(\\%)", 
                        "Leaf~Sugars~(\\%)", 
                        "Leaf~Nitrogen~(\\%)",
                        "Root~Nitrogen~(\\%)",
                        "SRL~(cm~m\\textsuperscript{-1})",
                        "{\\textdelta}\\textsuperscript{13}C~(\\text{\\textperthousand})")
...
%seedling data table
<<results='asis'>>=
datatab <- xtable(pve_data, label="table:Table1", caption="Responses of plant and leaf characterisitics of \\textit{Eucalyptus tereticornis} seedlings to soil volume treatments. Each value reflects the mean(standard error) of each treatment.")
