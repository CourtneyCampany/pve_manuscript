...
colnames(pve_data) <- c("\tmathrm{Volume~(L)}", "\t{Seedling~mass~(g)}",
                        ...
                        "$\\mathrm{Leaf~Respiration_{dark}~(\\mu~mols~m^{-2}~s^{-1})}$")
@


% dataframe for table1
<<results='asis'>>=
datatab <- xtable(pve_data, label="table:Table1")
print(datatab, include.rownames=FALSE,sanitize.colnames.function=function(x){x},
      floating.environment='sidewaystable',scalebox=0.7)
