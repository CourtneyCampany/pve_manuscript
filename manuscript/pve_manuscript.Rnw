...
colnames(pve_data) <- c("\t{Volume~(L)}", 
                        ...
                        "$\\mathrm{SLA~(m^2~g^{-1})}$",
                        "$\\mathrm{SRL~(m~g^{-1})}$", 
                        "$\\mathrm{Leaf~Starch~(g~g^{-1})}$", 
                        "$\\mathrm{Leaf~Sugars~(g~g^{-1})}$", 
                        "$\\mathrm{Leaf~Nitrogen~(g~g^{-1}))}$")


phys_data <- read.csv("table/pve_table2.csv")
colnames(phys_data) <- c("\t{Volume~(L)}", 
                        "$\\mathrm{A_{max}~(\\mu~mols~m^{-2}~s^{-1})}$", 
                        "$\\mathrm{R_{dark}~(\\mu~mols~m^{-2}~s^{-1})}$",
                        "$\\mathrm{J_{max}}$",
                        "$\\mathrm{Vc_{max}}$",
                        "$\\mathrm{g_s~(\\mu~mols~m^{-2}~s^{-1})}$",
                        "$\\mathrm{g_1}$")
...
%seedling data table
<<results='asis'>>=
datatab <- xtable(pve_data, label="table:Table1", caption="Responses of plant and leaf characterisitcs of \textit{Eucalyptus tereticornis} to soil volume treatments. Each value reflects the mean(standard error) of each treatment.")
...
<<results='asis'>>=
phystab <- xtable(phys_data, label="table:Table2",caption="Responses of leaf level gas exchange parameters of \textit{Eucalyptus tereticornis} to soil volume treatments. Each value reflects the mean(standard error) of each treatment.")
...
% \begin{table}[!h] 
%   \centering
%   \caption{Responses of leaf level gas exchange parameters of Eucalyptus tereticornis to soil volume treatments. Each value reflects the mean(standard error) of each treatment.}
%   \label{tab:table3}
% <<table3,echo=F,results=tex>>=
% phystab <- xtable(phys_data)
% 
% print(phystab, include.rownames=FALSE,sanitize.colnames.function=function(x){x},
%       floating.environment='sidewaystable',caption.placement="top")
% @
...
Figure.~\ref{fig:figure3}. Soil volume treatment means~$\pm$~SE~(n=7), across all measurement dates (n=6), of light saturated rates of photosynthesis at 25$\degree$C̊.
