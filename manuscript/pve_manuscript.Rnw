...
colnames(pve_data) <- c("Volume (L)", 
                        "Seedling~mass~(g)", 
                        "SLA~(m\\textsuperscript{2}~g\\textsuperscript{-1})",
                        "Leaf~Nitrogen~(mg~g\\textsuperscript{-1})",
                        "Leaf~Sugars~(\\%)", 
                        "Leaf~Starch~(\\%)", 
...
%seedling data table
<<results='asis'>>=
datatab <- xtable(pve_data, label="table:Table1", caption="Responses of plant and leaf characterisitcs of Eucalyptus tereticornis to soil volume treatments. Each value reflects the mean(standard error) of each treatment.")
